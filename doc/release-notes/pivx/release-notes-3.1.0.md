Bulldog Core version *3.1.0* is now available from:  <https://github.com/BulldogCrew/bulldog/releases>

This is a new major version release, including various bug fixes and performance improvements, as well as updated translations.

Please report bugs using the issue tracker at github: <https://github.com/BulldogCrew/bulldog/issues>


Mandatory Update
==============

Bulldog Core v3.1.0 is a mandatory update for all users. This release contains new consensus rules and improvements that are not backwards compatible with older versions. Users will have a grace period of one week to update their clients before enforcement of this update is enabled.
...
zbull Staking is here! zbull staking will be activated on the morning of the 8th of May 2018. With the release of zbull staking, there are effectively 2 versions of zbull, zbull minted on the 3.0.6 bulldog wallet or lower, and zbull minted on bulldog wallet version or higher. New features in this release will require the use of zbull v2, zbull minted on this wallet release 3.1.0 or later. If you currently hold zbull v1 and wish to take advantage of zbull staking and deterministic zbull, you will need to spend the zbull v1 to yourself and remint zbull v2.
Note: To find your zbull version, click the privacy tab, then the zbull Control button then expand the arrows next to the desired denomination.


### Deterministic zbull Seed Keys

zbull is now associated with a deterministic seed key. With this seed key, users are able to securely backup their zbull outside of the wallet that the zbull had been minted on. zbull can also be transferred from wallet to wallet without the need of transferring the wallet data file.


### Updated zbull minting

zbull minting now only requires 1 further mint (down from 2) to mature. zbull mints still require 20 confirmations.  Mints also require that the 'second' mint is at least two checkpoints deep in the chain (this was already the case, but the logic was not as precise).


### zbull Search

Users will now have the ability to search the blockchain for a specific serial # to see if a zbull denomination has been spent or not.



BULL/zbull Staking and Masternode Rewards
--------------

### BULL, zbull and Masternode Payment Schedule

To encourage the use of zbull and increase the bulldog zerocoin anonymity set, the bulldog payment schedule has been changed to the following:

If a user staking zbull wins the reward for their block, the following zbull reward will be: 
- 3 zbull (3 x 1 denominations) rewarded to the staker, 2 BULL rewarded to the masternode owner and 1 BULL available for the budget. This is a total block reward of 6 BULL, up from 5.

If a user staking BULL wins the reward, the following amounts will be rewarded: 
- 2 BULL to the BULL staker, 3 BULL to the Masternode owner and 1 BULL available for the budget. This is a total block reward of 6 BULL, up from 5.


### Return change to sender when minting zbull

Previously, zbull minting would send any change to a newly generated "change address". This has caused confusion among some users, and in some cases insufficient backups of the wallet. The wallet will now find the contributing address which contained the most BULL and return the change from a zbull mint to that address.
...
The visual layout of the bulldog Qt wallet has undergone a near-complete overhaul.
...
-	Enable and disable the auto zbull minting feature. This is enabled by default and the enablezeromint=0 setting in the bulldog.conf file will overwrite the GUI option.
-	The percentage of autominted zbull can now be set from 1 to 100, changed from 10 – 100.
-	The stake split threshold can now be set VIA the wallet options. This setting is an advanced feature for those wishing to remain staking regular BULL.
...
### Summary
 ...
 The bulldog wallet can now have user selected directories for automatic backups of the wallet data file (wallet.dat). This can be set by adding the following lines to the bulldog.conf file, found in the bulldog data directory.
- backuppath = <directory / full path>
- zbullbackuppath = <directory / full path>
...
* If zbull backup, auto generated name is `wallet-autozbullbackup.dat-<year>-<month>-<day>-<hour>-<minute>-<second>`
...
bulldog Daemon & Client (RPC Changes)
--------------
...
This allows running bulldogd without having to do any manual configuration.
...
Search the zerocoin database for a zerocoinspend transaction that contains the given serial. This will be a helpful tool for the bulldog support group, which often times sees users say "I didn't spend that zbull". This RPC call allows for support to grab the serial, and then find the spend tx on the chain.
...
As voted on by the bulldog masternodes, the monthly budget available to be utilised has been increased to 42,000 BULL / month. This BULL only has the opportunity to be raised once per month (paid to winning proposals) with any unused BULL not created by the blockchain.

### Budget Finalization Fee

The bulldog finalization fee for successful proposals has now been reduced, this fee is now 5 BULL down from 50 BULL. The total fee outlay for a successful proposal is now a total of 55 BULL.
...
-	Enforced v1 zbull spends to require a security level of 100
-	Updates to zbull spends to avoid segfaults
-	Updates to configuration will now reflect on the privacy tab
-	Fixed a  bug that would not start masternodes from the bulldog-Qt masternodes tab
-	Updated bulldog-Qt tooltips
...
Several critical security flaws in the zerocoin protocol and bulldog's zerocoin implementation have been patched. Enough has changed that new zerocoins are distinct from old zerocoins, and have been labelled as *version 2*. When using the zbull Control dialog in the QT wallet, a user is able to see zbull marked as version 1 or 2.

zPoS (zbull staking)
--------------

Once a zbull has over 200 confirmations it becomes available to stake. Staking zbull will consume the exact zerocoin that is staked and replace it with a freshly minted zerocoin of the same denomination as well as a reward of three 1 denomination zbull. So for example if a 1,000 zbull denomination is staked, the protocol replaces that with a fresh 1,000 denomination and three1 denomination zbulls.

Secure Spending
--------------

Version 1 zerocoins, as implemented by [Miers et. al](http://zerocoin.org/media/pdf/ZerocoinOakland.pdf), allow for something we describe as *serial trolling*. Spending zerocoins requires that the spender reveal their serial number associated with the zerocoin, and in turn that serial number is used to check for double spending. There is a fringe situation (which is very unlikely to happen within bulldog's zerocoin implementation due to delayed coin accumulation) where the spender sends the spending transaction, but the transaction does not immediately make it into the blockchain and remains in the mempool for a long enough duration that a *troll* has enough time to see the spender's serial number, mint a new zerocoin with the same serial number, and spend the new zerocoin before the original spender's transaction becomes confirmed. If the timing of this fringe situation worked, then the original spender's coin would be seen as invalid because the troll was able to have the serial recorded into the blockchain first, thus making the original spender's serial appear as a double spend.
...
Zerocoins, or zbull, are now deterministically generated using a unique 256 bit seed. Each wallet will generate a new seed on its first run. The deterministic seed is used to generate a string of zbull that can be recalculated at any time using the seed. Deterministic zbull allows for users to backup all of their future zbull by simply recording their seed and keeping it in a safe place (similar to backing up a private key for BULL). The zbull seed needs to remain in the wallet in order to spend the zbull after it is generated, if the seed is changed then the coins will not be spendable because the wallet will not have the ability to regenerate all of the private zbull data from the seed. It is important that users record & backup their seed after their first run of the wallet. If the wallet is locked during the first run, then the seed will be generated the first time the wallet is unlocked.

Zerocoin Modulus
--------------

bulldog's zerocoin implementation used the same code from the ZCoin project to import the modulus use for the zerocoin protocol. The chosen modulus is the 2048 bit RSA number created for the RSA factoring challenge. The ZCoin project's implementation (which bulldog used) improperly imported the modulus into the code. This flaw was discovered by user GOAT from the [Civitas Project](https://github.com/eastcoastcrypto/Civitas/), and reported to bulldog using the bug bounty program. The modulus is now correctly imported and bulldog's accumulators have been changed to use the new proper modulus.
...
### Core Features
 - #582 `cb1632520` [Core] zbull v2: zPoS dzbull ezbull external backup and budget fixes (too many to list)
...
### GUI
 - #572 `d9b23fe60` [Qt] Refresh zbull balance after resetting mints or spends (warrows)
 ...
 - #506 `ae72bf4e2` [Qt] Autoscroll to end of zbull status output (Mrs-X)
 ...
 - #467 `7099831bd` [Qt] Clear PrivacyDialog "zbull Selected" labels after sending. (presstab)
...
### RPC/REST
 ...
 - #543 `e4522ff07` [RPC] Segfault bulldog-cli getinfo while loading block index (Mrs-X)
...
### Wallet
 - #570 `8c8350b59` [Wallet] Add a check on zbull spend to avoid a segfault (warrows)
 ...
 - #448 `222ef6e6b` [Wallet] Return change to sender when minting zbull. (presstab)
 - #445 `fc570fc1e` [Wallet] Only require 1 mint to be added before spending zbull. (presstab)
...
As well as everyone that helped translating on [Transifex](https://www.transifex.com/projects/p/bulldog-project-translations/), the QA team during Testing and the Node hosts supporting our Testnet.

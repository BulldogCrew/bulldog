...
rm /usr/local/bin/bull*
git clone https://github.com/Liquid369/bulldog
cd bulldog/src
cd ..
prepare_system
create_swap
...
make install
if [ -d '/root/.bulldog' ] ; then
#Things to do
cd /root/.bulldog
rm -rf *
touch bulldog.conf
echo "rpcuser=user" >> bulldog.conf
echo "rpcpassword=pass123" >> bulldog.conf
echo "server=1" >> bulldog.conf
echo "daemon=1" >> bulldog.conf
echo "listen=1" >> bulldog.conf
echo "rpcallowip=127.0.0.1" >> bulldog.conf

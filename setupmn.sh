...
TMP_FOLDER=$(mktemp -d)
CONFIG_FILE='bulldog.conf'
CONFIGFOLDER='/root/.bulldog'
...
printf "Hello Bull,  \n Enter 1 for Install BullNode \n Enter 2 for Update BullNode \n Enter 3 for bootstrap. \n "
read choice
if (($choice == 1 )) 
 then
	cleanup_mess
 ...
 then
	...
	echo -e "${GREEN}BullNode Updated.${NC}"
elif (($choice == 3 ))
 then
	backup
	blocks
...
# unzip bulldog.zip
tar -xzf Bulldog-4.0.0-x86_64-linux-gnu.tar.gz
cd Bulldog-4.0.0/bin
chmod -R 775 *
cp * $COIN_PATH
cd ..
cd $COIN_PATH
...
function blocks() {
	wget https://github.com/bulldog/bulldog/raw/master/blocks.sh
	chmod 777 blocks.sh 
	bash blocks.sh
...
addnode=155.138.196.54:26740
addnode=155.138.210.119:26740
addnode=51.75.248.126:26740
listen=1
server=1
daemon=1
port=$COIN_PORT
...
function save_key() {
	cd /root/.bulldog
	mv /root/.bulldog/bulldog.conf /root/.bulldog/bulldog_old.conf
	cp /root/.bulldog/bulldog_old.conf /root
}

function restore_key() {
	cd /root/.bulldog
	rm masternode.conf
	cp /root/bulldog_old.conf /root/.bulldog
	mv /root/.bulldog/bulldog_old.conf /root/.bulldog/bulldog.conf
}
...
function backup() {
	...
	rm -rf blocks-
	rm blocks.sh
	zip -r backupdg.zip /root/.bulldog
	cp /root/.bulldog/backupdg.zip /root
...
#  wget https://gist.githubusercontent.com/hoserdude/9661c9cdc4b59cf5f001/raw/5972d4d838691c1a1f33fb274f97fa0b403d10bd/bull.txt
  cat bull.txt
printf "%s\n"
echo "Bulldog MN installer Depends Starting"

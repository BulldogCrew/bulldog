...
bool CheckProofOfStake(const CBlock block, uint256& hashProofOfStake, std::unique_ptr<CStakeInput>& stake)
{
    ...
    if (tx.IsZerocoinSpend()) {
        ...
        stake = std::unique_ptr<CStakeInput>(new CzbullStake(spend));
    } else {
        ...
        CBULLStake* BULLInput = new CBULLStake();
        BULLInput->SetInput(txPrev, txin.prevout.n);
        stake = std::unique_ptr<CStakeInput>(BULLInput);
    }

...
void OptionsModel::Init()
{
    ...
    if (!settings.contains("nDisplayUnit"))
        settings.setValue("nDisplayUnit", BitcoinUnits::BULL);
    ...
    if (!settings.contains("nAnonymizebulldogAmount"))
        settings.setValue("nAnonymizebulldogAmount", 1000);

    nAnonymizebulldogAmount = settings.value("nAnonymizebulldogAmount").toLongLong();
...
// Wallet
    ...
    if (settings.contains("nAnonymizebulldogAmount"))
        SoftSetArg("-anonymizebulldogamount", settings.value("nAnonymizebulldogAmount").toString().toStdString());
...
void OptionsModel::Reset()
{
    ...
    settings.clear();
    resetSettings = true; // Needed in bulldog.cpp during shotdown to also remove the window positions
...
QVariant OptionsModel::data(const QModelIndex& index, int role) const
{
    if (role == Qt::EditRole) {
        ...
        case AnonymizebulldogAmount:
            return QVariant(nAnonymizebulldogAmount);
        case Listen:
...
bool OptionsModel::setData(const QModelIndex& index, const QVariant& value, int role)
{
    ...
    if (role == Qt::EditRole) {
        ...
        case HideOrphans:
            ...
            break;
        case AnonymizebulldogAmount:
            nAnonymizebulldogAmount = value.toInt();
            settings.setValue("nAnonymizebulldogAmount", nAnonymizebulldogAmount);
            emit anonymizebulldogAmountChanged(nAnonymizebulldogAmount);
            break;
        case CoinControlFeatures:

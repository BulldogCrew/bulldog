...
#include "ui_zbullcontroldialog.h"
...
std::set<std::string> zbullControlDialog::setSelectedMints;
std::set<CMintMeta> zbullControlDialog::setMints;

bool CzbullControlWidgetItem::operator<(const QTreeWidgetItem &other) const {
    int column = treeWidget()->sortColumn();
    if (column == zbullControlDialog::COLUMN_DENOMINATION || column == zbullControlDialog::COLUMN_VERSION || column == zbullControlDialog::COLUMN_CONFIRMATIONS)
        return data(column, Qt::UserRole).toLongLong() < other.data(column, Qt::UserRole).toLongLong();
    return QTreeWidgetItem::operator<(other);
}


zbullControlDialog::zbullControlDialog(QWidget *parent) :
    QDialog(parent, Qt::WindowSystemMenuHint | Qt::WindowTitleHint | Qt::WindowCloseButtonHint),
    ui(new Ui::zbullControlDialog),
    model(0)
{
    ui->setupUi(this);
    setMints.clear();
...
zbullControlDialog::~zbullControlDialog()
{
    delete ui;
}

void zbullControlDialog::setModel(WalletModel *model)
{
    this->model = model;
    updateList();
}
...
void zbullControlDialog::updateList()
{
    ...
    for (auto denom : libzerocoin::zerocoinDenomList) {
        CzbullControlWidgetItem* itemDenom(new CzbullControlWidgetItem);
    ...
    for (const CMintMeta& mint : setMints) {
        ...
        CzbullControlWidgetItem *itemMint = new CzbullControlWidgetItem(ui->treeWidget->topLevelItem(mapDenomPosition.at(denom)));
        ...
        {
            LOCK(pwalletMain->zbullTracker->cs_spendcache);

            CoinWitnessData *witnessData = pwalletMain->zbullTracker->GetSpendCache(mint.hashStake);
        ...
        if(!fSpendable) {
            ...
            if(nConfirmations < Params().Zerocoin_MintRequiredConfirmations())
                ...
                strReason = "Your wallet is locked. Impossible to precompute or spend zbull.";
            else if (!mint.isSeedCorrect)
                strReason = "The zbull seed used to mint this zbull is not the same as currently hold in the wallet";
            else
...
void zbullControlDialog::updateSelection(QTreeWidgetItem* item, int column)
{
...
void zbullControlDialog::updateLabels()
{
    int64_t nAmount = 0;
    for (const CMintMeta& mint : setMints) {
    ...
    ui->labelzbull_int->setText(QString::number(nAmount));
    ui->labelQuantity_int->setText(QString::number(setSelectedMints.size()));

    //update PrivacyDialog labels
    privacyDialog->setzbullControlLabels(nAmount, setSelectedMints.size());
}

std::vector<CMintMeta> zbullControlDialog::GetSelectedMints()
{
...
void zbullControlDialog::ButtonAllClicked()
{
    ui->treeWidget->blockSignals(true);

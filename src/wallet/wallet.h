...
extern bool fGlobalUnlockSpendCache; // Bool used for letting the precomputing thread know that zbullspends need to use the cs_spendcache
...
enum AvailableCoinsType {
    ...
    ONLY_DENOMINATED = 2,
    ONLY_NOT10000IFMN = 3,
    ONLY_NONDENOMINATED_NOT10000IFMN = 4, // ONLY_NONDENOMINATED and not 10000 BULL at the same time
    ONLY_10000 = 5,                        // find masternode outputs including locked ones (use with caution)
    STAKABLE_COINS = 6                          // UTXO's that are valid for staking
};

// Possible states for zbull send
enum ZerocoinSpendStatus {
    zbull_SPEND_OKAY = 0,                            // No error
    zbull_SPEND_ERROR = 1,                           // Unspecified class of errors, more details are (hopefully) in the returning text
    zbull_WALLET_LOCKED = 2,                         // Wallet was locked
    zbull_COMMIT_FAILED = 3,                         // Commit failed, reset status
    zbull_ERASE_SPENDS_FAILED = 4,                   // Erasing spends during reset failed
    zbull_ERASE_NEW_MINTS_FAILED = 5,                // Erasing new mints during reset failed
    zbull_TRX_FUNDS_PROBLEMS = 6,                    // Everything related to available funds
    zbull_TRX_CREATE = 7,                            // Everything related to create the transaction
    zbull_TRX_CHANGE = 8,                            // Everything related to transaction change
    zbull_TXMINT_GENERAL = 9,                        // General errors in MintToTxIn
    zbull_INVALID_COIN = 10,                         // Selected mint coin is not valid
    zbull_FAILED_ACCUMULATOR_INITIALIZATION = 11,    // Failed to initialize witness
    zbull_INVALID_WITNESS = 12,                      // Spend coin transaction did not verify
    zbull_BAD_SERIALIZATION = 13,                    // Transaction verification failed
    zbull_SPENT_USED_zbull = 14,                      // Coin has already been spend
    zbull_TX_TOO_LARGE = 15,                          // The transaction is larger than the max tx size
    zbull_SPEND_V1_SEC_LEVEL                         // Spend is V1 and security level is not set to 100
};
...
public:
    ...
    void zbullBackupWallet();
    bool GetZerocoinKey(const CBigNum& bnSerial, CKey& key);
    bool CreatezbullOutPut(libzerocoin::CoinDenomination denomination, CTxOut& outMint, CDeterministicMint& dMint);
    bool GetMint(const uint256& hashSerial, CZerocoinMint& mint);
    bool GetMintFromStakeHash(const uint256& hashStake, CZerocoinMint& mint);
    bool DatabaseMint(CDeterministicMint& dMint);
    bool SetMintUnspent(const CBigNum& bnSerial);
    bool UpdateMint(const CBigNum& bnValue, const int& nHeight, const uint256& txid, const libzerocoin::CoinDenomination& denom);
    string GetUniqueWalletBackupName(bool fzbullAuto) const;
    ...
    */
    ...
    bool fBackupMints;
    std::unique_ptr<CzbullTracker> zbullTracker;
    ...
    void setZWallet(CzbullWallet* zwallet)
    {
        zwalletMain = zwallet;
        zbullTracker = std::unique_ptr<CzbullTracker>(new CzbullTracker(strWalletFile));
    }

    CzbullWallet* getZWallet() { return zwalletMain; }
    ...
    void setzbullAutoBackups(bool fEnabled)
    {
    ...
    /// Get 5000BULL input and keys which can be used for the Masternode
    ...
    /** zbull reset */
    boost::signals2::signal<void()> NotifyzbullReset;

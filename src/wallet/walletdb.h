...
public:
    ...
    bool WritezbullSeed(const uint256& hashSeed, const vector<unsigned char>& seed);
    bool ReadzbullSeed(const uint256& hashSeed, vector<unsigned char>& seed);
    bool ReadzbullSeed_deprecated(uint256& seed);
    bool ErasezbullSeed();
    bool ErasezbullSeed_deprecated();

    bool WritezbullCount(const uint32_t& nCount);
    bool ReadzbullCount(uint32_t& nCount);

...
CzbullTracker::CzbullTracker(std::string strWalletFile)
{
...
CzbullTracker::~CzbullTracker()
{
    mapSerialHashes.clear();
    mapPendingSpends.clear();
}

void CzbullTracker::Init()
{
...
bool CzbullTracker::Archive(CMintMeta& meta)
{
...
bool CzbullTracker::UnArchive(const uint256& hashPubcoin, bool isDeterministic)
{
    CWalletDB walletdb(strWalletFile);
    if (isDeterministic) {
        CDeterministicMint dMint;
...
CMintMeta CzbullTracker::Get(const uint256 &hashSerial)
{
...
CMintMeta CzbullTracker::GetMetaFromPubcoin(const uint256& hashPubcoin)
{
...
bool CzbullTracker::GetMetaFromStakeHash(const uint256& hashStake, CMintMeta& meta) const
{
    for (auto& it : mapSerialHashes) {
...
CoinWitnessData* CzbullTracker::GetSpendCache(const uint256& hashStake)
{
...
bool CzbullTracker::ClearSpendCache()
{
    AssertLockHeld(cs_spendcache);
...
std::vector<uint256> CzbullTracker::GetSerialHashes()
{
...
CAmount CzbullTracker::GetBalance(bool fConfirmedOnly, bool fUnconfirmedOnly) const
{
    CAmount nTotal = 0;
...
CAmount CzbullTracker::GetUnconfirmedBalance() const
{
    return GetBalance(false, true);
}

std::vector<CMintMeta> CzbullTracker::GetMints(bool fConfirmedOnly) const
{
...
bool CzbullTracker::HasMintTx(const uint256& txid)
{
    for (auto it : mapSerialHashes) {
...
bool CzbullTracker::HasPubcoin(const CBigNum &bnValue) const
{
...
bool CzbullTracker::HasPubcoinHash(const uint256& hashPubcoin) const
{
    for (auto it : mapSerialHashes) {
        CMintMeta meta = it.second;
        if (meta.hashPubcoin == hashPubcoin)
...
bool CzbullTracker::HasSerial(const CBigNum& bnSerial) const
{
    uint256 hash = GetSerialHash(bnSerial);
    return HasSerialHash(hash);
}

bool CzbullTracker::HasSerialHash(const uint256& hashSerial) const
{
    auto it = mapSerialHashes.find(hashSerial);
    return it != mapSerialHashes.end();
}

bool CzbullTracker::UpdateZerocoinMint(const CZerocoinMint& mint)
{
...
bool CzbullTracker::UpdateState(const CMintMeta& meta)
{
    CWalletDB walletdb(strWalletFile);
...
void CzbullTracker::Add(const CDeterministicMint& dMint, bool isNew, bool isArchived, CzbullWallet* zbullWallet)
{
    bool iszbullWalletInitialized = (NULL != zbullWallet);
    CMintMeta meta;
    ...
    if (! iszbullWalletInitialized)
        zbullWallet = new CzbullWallet(strWalletFile);
    meta.isSeedCorrect = zbullWallet->CheckSeed(dMint);
    if (! iszbullWalletInitialized)
        delete zbullWallet;
    mapSerialHashes[meta.hashSerial] = meta;
...
void CzbullTracker::Add(const CZerocoinMint& mint, bool isNew, bool isArchived)
{
    CMintMeta meta;
...
void CzbullTracker::SetPubcoinUsed(const uint256& hashPubcoin, const uint256& txid)
{
...
void CzbullTracker::SetPubcoinNotUsed(const uint256& hashPubcoin)
{
    if (!HasPubcoinHash(hashPubcoin))
        return;
    CMintMeta meta = GetMetaFromPubcoin(hashPubcoin);
...
void CzbullTracker::RemovePending(const uint256& txid)
{
    uint256 hashSerial;
...
bool CzbullTracker::UpdateStatusInternal(const std::set<uint256>& setMempool, CMintMeta& mint)
{
...
std::set<CMintMeta> CzbullTracker::ListMints(bool fUnusedOnly, bool fMatureOnly, bool fUpdateStatus, bool fWrongSeed)
{
    CWalletDB walletdb(strWalletFile);
    if (fUpdateStatus) {
        ...
        CzbullWallet* zbullWallet = new CzbullWallet(strWalletFile);
        for (auto& dMint : listDeterministicDB) {
            Add(dMint, false, false, zbullWallet);
        }
        delete zbullWallet;
        LogPrint("zero", "%s: added %d dzbull from DB\n", __func__, listDeterministicDB.size());
    }

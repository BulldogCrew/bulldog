...
CzbullWallet::CzbullWallet(std::string strWalletFile)
{
    ...
    //Check for old db version of storing zbull seed
    if (fFirstRun) {
        uint256 seed;
        if (walletdb.ReadzbullSeed_deprecated(seed)) {
            //Update to new format, erase old
            seedMaster = seed;
            hashSeed = Hash(seed.begin(), seed.end());
            if (pwalletMain->AddDeterministicSeed(seed)) {
                if (walletdb.ErasezbullSeed_deprecated()) {
                    LogPrintf("%s: Updated zbull seed databasing\n", __func__);
                    fFirstRun = false;
                } else {
                    LogPrintf("%s: failed to remove old zbull seed\n", __func__);
                }
            }
        }
    }
    ...
    if (fFirstRun) {
        ...
        seedMaster = seed;
        LogPrintf("%s: first run of zbull wallet detected, new seed generated. Seedhash=%s\n", __func__, Hash(seed.begin(), seed.end()).GetHex());
...
bool CzbullWallet::SetMasterSeed(const uint256& seedMaster, bool fResetCount)
{
    ...
    if (fResetCount)
        walletdb.WritezbullCount(nCountLastUsed);
    else if (!walletdb.ReadzbullCount(nCountLastUsed))
        nCountLastUsed = 0;
...
void CzbullWallet::Lock()
{
    seedMaster = 0;
}

void CzbullWallet::AddToMintPool(const std::pair<uint256, uint32_t>& pMint, bool fVerbose)
{
    mintPool.Add(pMint, fVerbose);
}

//Add the next 20 mints to the mint pool
void CzbullWallet::GenerateMintPool(uint32_t nCountStart, uint32_t nCountEnd)
{
    ...
    for (uint32_t i = n; i < nStop; ++i) {
        ...
        CKey key;
        SeedTozbull(seedZerocoin, bnValue, bnSerial, bnRandomness, key);
...
bool CzbullWallet::LoadMintPoolFromDB()
{
...
void CzbullWallet::RemoveMintsFromPool(const std::vector<uint256>& vPubcoinHashes)
{
    for (const uint256& hash : vPubcoinHashes)
        mintPool.Remove(hash);
}

void CzbullWallet::GetState(int& nCount, int& nLastGenerated)
{
...
void CzbullWallet::SyncWithChain(bool fGenerateMintPool)
{
    ...
    while (found) {
        ...
        for (pair<uint256, uint32_t> pMint : listMints) {
            ...
            if (pwalletMain->zbullTracker->HasPubcoinHash(pMint.first)) {
...
bool CzbullWallet::SetMintSeen(const CBigNum& bnValue, const int& nHeight, const uint256& txid, const CoinDenomination& denom)
{
    ...
    CBigNum bnRandomness;
    CKey key;
    SeedTozbull(seedZerocoin, bnValueGen, bnSerial, bnRandomness, key);
    ...
    // Add to zbullTracker which also adds to database
    pwalletMain->zbullTracker->Add(dMint, true);
...
void CzbullWallet::SeedTozbull(const uint512& seedZerocoin, CBigNum& bnValue, CBigNum& bnSerial, CBigNum& bnRandomness, CKey& key)
{
...
uint512 CzbullWallet::GetZerocoinSeed(uint32_t n)
{
...
void CzbullWallet::GenerateDeterministiczbull(CoinDenomination denom, PrivateCoin& coin, CDeterministicMint& dMint, bool fGenerateOnly)
{
...
void CzbullWallet::GenerateMint(const uint32_t& nCount, const CoinDenomination denom, PrivateCoin& coin, CDeterministicMint& dMint)
{
...
bool CzbullWallet::CheckSeed(const CDeterministicMint& dMint)
{
...
bool CzbullWallet::RegenerateMint(const CDeterministicMint& dMint, CZerocoinMint& mint)
{
    if (!CheckSeed(dMint)) {
        uint256 hashSeed = Hash(seedMaster.begin(), seedMaster.end());
